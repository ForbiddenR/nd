...
Relative contexts are resolved from the directory where `nd` was launched. A
build context must contain `Dockerfile` or `Containerfile`; `nd` passes the first
one it finds to `nerdctl` with `-f`. The context is passed directly to `nerdctl`
as one argument, so spaces are supported but shell expansion such as `~`, `$VAR`,
or `*` is not performed.
